...
export default function CountdownTimer({ targetDate }: CountdownTimerProps) {
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="text-sm uppercase tracking-widest mb-4 font-light">Do svatby zbývá</div>
        ...
        {timeUnits.map((unit, index) => (
          <div key={index} className="flex flex-col items-center">
            <div className="bg-white/20 backdrop-blur-sm rounded-lg w-16 md:w-20 h-16 md:h-20 flex items-center justify-center border border-white/30">
              <span className="text-2xl md:text-3xl font-light">{unit.value || 0}</span>
            </div>
            <span className="text-xs uppercase tracking-wider mt-2 font-light">{unit.label}</span>
          </div>
        ))}
      </div>
    </div>
